...
def loop():
  ...
  while True:
    ...
    if (not marioMode) and currentUser:
      ...
      LCD.lcd_string( str( int(round( (currentUserTime - time.time())/3600 ))) + ":" + str( int(round( (currentUserTime - time.time())/60 ))) + ":" + str( int(round( (currentUserTime - time.time())%60 ))) ,LCD.LCD_LINE_2)
